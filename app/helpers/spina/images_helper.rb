module Spina
  module ImagesHelper
    ...
    def thumbnail_url(image)
      return "" if image.nil?
      main_app.url_for(image.variant(format: :webp, saver: {quality: 70}, resize_to_fill: [400, 300]))
    end
    ...
    def embedded_image_url(image)
      ...
      main_app.url_for(image.variant(format: :webp, saver: {quality: 70}, Hash[resize_key, Spina.config.embedded_image_size]))
    end
